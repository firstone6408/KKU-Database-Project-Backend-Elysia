...
export abstract class BranchService {
  public static async createBranch(options: {
    ...
    const existingBranch = await db.branch.findFirst({
      where: {
        OR: [{ branchCode: options.branchCode }, { name: options.name }],
      },
      select: { id: true },
    });
  ...
  public static async updateBranch(
    ...
    const branch = await db.branch.findFirst({
      where: {
        OR: [{ name: options.name }, { branchCode: options.branchCode }],
      },
    ...
    if (
      ...
      branch.branchCode !== options.branchCode
    ) {
      throw new HttpError({
        statusCode: 400,
