...
export abstract class PaymentMethodService
{
    public static async createPaymentMethod(options: { name: string })
    {
        ...
        const created = await db.paymentMethod.create({ data: options, select: { name: true } });

        return created;
    }

    public static async updatePaymentMethod(id: string, options: { name: string })
    {
        const existingPaymentMethod = await db.paymentMethod.findUnique(
            {
                where: { name: options.name },
    ...
    {
        await db.paymentMethod.delete({ where: { id: id }, select: { id: true } }).catch(
            _ =>
            {
                throw new HttpError(
                    {
