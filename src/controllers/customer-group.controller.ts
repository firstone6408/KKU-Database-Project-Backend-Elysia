...
export const customerGroupCotroller = new Elysia({
        ...
        .post(
          ...
          ({ body, set }) =>
            withRequestHandling(async () => {
              const customerGroup =
                await CustomerGroupService.createCustomerGroup(body);
              set.status = "Created";
              return {
                payload: { data: null },
                message: `เพิ่มกลุ่ม ${customerGroup.name} สำเร็จ`,
              };
            }),
      ...
      app.get("/", () =>
        withRequestHandling(async () => {
          const customerGroups =
            await CustomerGroupService.listCustomerGroup();
